...
if summary:
    ...
    st.sidebar.markdown(f"**Breakouts:** {summary.get('breakouts', 0)}")
...
col2.metric("Tier S", (filtered["technical_tier"] == "S").sum())
col3.metric("Tier A", (filtered["technical_tier"] == "A").sum())
...
breakouts = filtered[filtered["breakout"] == True].sort_values("technical_score", ascending=False)
if not breakouts.empty:
    bo_display = breakouts[["rank", "display", "technical_tier", "technical_score", "price", "breakout_vol_ratio", "fundamental_tag"]].copy()
    bo_display.columns = ["Rank", "Ticker", "Tier", "Score Téc.", "Preço", "Ratio Vol", "Fund."]
...
st.dataframe(
    display_df.style.applymap(color_tier, subset=["Tier"]).applymap(color_fund_tag, subset=["Tag Fund."]),
...
# Footer
st.markdown("---")
st.caption(f"Screening B3 v2.0 | Atualizado: {summary.get('date', 'N/A')} | Desenvolvido por marcelocv8")
